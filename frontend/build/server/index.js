import {
  ...
  m as setContext,
  n as pop$1,
  q as decode_pathname,
  t as decode_params,
  u as normalize_path,
  v as disable_search,
  w as validate_layout_server_exports,
  x as validate_layout_exports,
  y as validate_page_server_exports,
  z as validate_page_exports,
  A as resolve,
  B as make_trackable,
  C as readable,
  D as writable,
} from "./chunks/exports-DunOQ8il.js";
    ...
    value,
    {
      ...
      get(target, prop, receiver) {
        ...
        if (
          ...
          s = with_parent(() =>
            state(proxy(exists ? target[prop] : UNINITIALIZED)),
          );
          sources.set(prop, s);
        }
        if (s !== void 0) {
      ...
      has(target, prop) {
        ...
        if (
          ...
          if (s === void 0) {
            s = with_parent(() =>
              state(has ? proxy(target[prop]) : UNINITIALIZED),
            );
            sources.set(prop, s);
          }
          var value2 = get(s);
      ...
      fetch: async (info2, init3) => {
        ...
        if (set_cookie) {
          for (const str of setCookieExports.splitCookiesString(set_cookie)) {
            const { name, value, ...options3 } = setCookieExports.parseString(
              str,
              {
                decodeValues: false,
              },
